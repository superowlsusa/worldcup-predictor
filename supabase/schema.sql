...
begin
  -- Lock per stage 1 hour before that stage's first match. Only enforced when a
  -- player creates/changes their pick; system point updates pass through.
  if tg_op = 'INSERT'
...
-- Split tables: group-stage games only, and knockout games only.
create or replace view public.standings_group as
select p.user_id, coalesce(pr.display_name, 'Player') as display_name,
  sum(p.points)::integer as total_points,
  count(*) filter (where p.points = 3)::integer as exact_scores,
  count(*) filter (where p.points = 1)::integer as correct_outcomes,
  count(*)::integer as predictions_made
